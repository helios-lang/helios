...
pub struct Receiver {
    ...
    syntax_tree_database: HashMap<String, (i64, Ast)>,
}

impl Receiver {
    pub fn with(responder_channel: Sender<LspResponse>) -> Self {
        Receiver { responder_channel, syntax_tree_database: HashMap::new(), }
    }

    fn generate_syntax_tree(&self, path: Url) -> Result<Ast, String> {
        match path.to_file_path() {
            Ok(path) => match Source::file(path) {
                Ok(source) => Ok(koi_driver::parse(source)),
    ...
    fn cache_syntax_tree<V: Into<Option<i64>>>(&mut self, url: Url, version: V) {
        let version = version.into().unwrap_or(0);
        let key = url.to_string();
        match self.syntax_tree_database.get(&key) {
            Some((cached_version, _)) => {
                if cached_version != &version {
                    match self.generate_syntax_tree(url) {
                        Ok(tokens) => {
                            self.syntax_tree_database.insert(key, (version, tokens));
                        },
                        Err(error) => {
    ...
    fn process_message(&mut self, message: LspMessage) {
        match message {
            ...
            LspMessage::TextDocumentDidOpenNotification { params, .. } => {
                ...
                self.cache_syntax_tree(uri.clone(), version);
